  ...
  users.forEach(user => {
    ...
    row.innerHTML = `
      ...
      <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
        ...
        <button onclick="sendCredentials(${user.id}, '${user.email}', '${user.name}')" class="text-green-600 hover:text-green-900" title="Enviar credenciais por email">
...
function sendCredentials(userId, email, name) {
  if (!confirm(`Enviar credenciais para ${name} (${email})?\n\nUma nova senha temporária será gerada e enviada por email.`)) {
    return;
  }
  ...
  fetch('/admin/users/send-credentials', {
    method: 'POST',
    headers: {
  ...
  .then(result => {
    if (result.success) {
      alert('Credenciais enviadas com sucesso para ' + email + '!');
    } else {
      alert('Erro ao enviar credenciais: ' + result.message);
    }
  })
  .catch(error => {
    alert('Erro de conexão: ' + error.message);
